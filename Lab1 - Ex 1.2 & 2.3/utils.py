...
def train(
    ...
    for epoch in range(start, n_epochs):
        ...
        if val_loader and (epoch % eval_interval == 0 or epoch == n_epochs - 1):
            ...
            if patience:
                ...
                if patience(val_metrics['MulticlassAccuracy']):
                    ...
                    best_epoch = epoch
                    best_train_loss = epoch_loss_norm
                    best_train_metrics = epoch_metrics
                    best_val_loss = val_loss_norm
                    best_val_metrics = val_metrics

                    wb_log['best_epoch'] = best_epoch
                    best_checkpoint = {'state_dict': model.state_dict(), 'optimizer': optimizer.state_dict()}
                    
                    save_model(best_checkpoint, cfg['model_path'], 'model')
        ...
        # Save checkpoint
        save_model(checkpoint, cfg['checkpoint_path'], 'checkpoint')
    ...
    if patience:
        train_results.update({'best_epoch': best_epoch, 'best_train_loss': best_train_loss})
        train_results.update(
            {f'best_train_{name}': metric for name, metric in zip(['accuracy', 'precision', 'recall'], best_train_metrics.values())}
        )
    ...
    if val_loader:
        ...
        if patience:
            train_results.update({'best_val_loss': best_val_loss})
            train_results.update(
                {f'best_val_{name}': metric for name, metric in zip(['accuracy', 'precision', 'recall'], best_val_metrics.values())}
            )
