...
class MNIST_Dataset(BaseDataset):
    def __init__(
        ...
        if val_size:
            ...
            if problem.lower() == 'od':
            n_train = len(self.val_set)  # val_set e non train_set perché dal train_set ho tolto le immagini OOD
...
class FashionMNIST_Dataset(BaseDataset):
    def __init__(self, 
        ...
        if problem is not None:
            if problem.lower() == 'od':
                self.n_classes = num_classes  # 0: normal, 1: outlier
                self.normal_classes = tuple([normal_class])  # tuple with original class labels that define the normal class
        ...
        self.train_set = MyFashionMNIST(root=self.root, train=True, download=True, transform=transform, target_transform=target_transform)
        self.test_set = MyFashionMNIST(root=self.root, train=False, download=True, transform=transform, target_transform=target_transform)
        self.org_test_set = MyFashionMNIST(root=self.root, train=False, download=True, transform=basic_transform, target_transform=target_transform)
        ...
        if val_size:
            self.val_set = MyFashionMNIST(root=self.root, train=True, download=True, transform=transform, target_transform=target_transform)
            n_train = len(self.val_set)  # val_set e non train_set perché dal train_set ho tolto le immagini OOD
            indices = list(range(n_train))
            split = int(np.floor(val_size * n_train))
            if val_shuffle:
